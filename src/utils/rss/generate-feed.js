...
const generateItem = function generateItem(post, settings, config) {
    ...
    const featureImgSharp =
        post.featureImageSharp && post.featureImageSharp.publicURL;
    const featureImgUrl =
        (featureImgSharp && url.resolve(config.siteUrl, featureImgSharp)) ||
        post.feature_image;
...
const generateRSSFeed = function generateRSSFeed(siteConfig) {
  ...
  `,
        output: `/rss`,
        title: `Daily Struggles RSS Feed`,
    };
};
